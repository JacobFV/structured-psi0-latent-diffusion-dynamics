...
marker="$RRP_PEER_REPO/$ROOT/.slot_done_${method}_$(echo "$@" | tr ' ' _)_b$(echo $BUDGETS | tr ' ' _)"
for seg in $(seq 1 20); do
  ...
  scripts/peer_run.sh --gpu --gpu-mem 16G --cpu 4 --mem 24G --label "baselines_${method#baseline_}" --max-seconds 21000 \
    -- env PY=/dev/shm/rrp-brandonin/venv/bin/python ROOT="$ROOT" BUDGETS="$BUDGETS" bash scripts/baselines_slot.sh "$method" "$@" 2>&1 \
