...
for attempt in 1 2 3; do
  if git fetch -q origin && { git rebase -q origin/main || scripts/demo/resolve.sh; } && python3 scripts/demo/build_page.py >/dev/null && { git add -A docs/demo; git diff --cached --quiet || git commit -q -m "demo: rebuild after rebase" -m "Co-Authored-By: Claude Opus 5.5 <noreply@anthropic.com>"; } && git push -q origin HEAD:main; then git push -q -f origin HEAD:track/demo; break; fi
  sleep 5
done
git log --oneline -1
