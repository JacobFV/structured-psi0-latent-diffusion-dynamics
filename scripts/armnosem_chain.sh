#!/usr/bin/env bash
# ARM NOSEM COUNTERPART (research/tracks/ladder.md): the frozen arm route's full recipe (D-072/D-078) replayed with a
# NO-SEMANTIC Stage A (configs/ladder/armnosem/*, generated from the sem configs by name substitution; only
# semantic_weight 0 in Stage A and packet_semantic_weight 0 in the flows differ). Same steps, seeds, DAgger seeds,
# body groups and buffer compositions as the sem lineage (read from the sem buffers' summaries).
# Runs ON THE PEER (from /dev/shm/rrp-brandonin/wt/ladder) as a DAG: every node waits for its dependencies' .done
# markers, runs ONE leased job (no retry), checks the exit code / outputs and writes .done or .failed.
# Resume: rerun this script; completed nodes are skipped (their .done exists). A .failed node blocks its dependants;
# inspect its log, delete the .failed marker, and rerun (manual decision, D-061).
# Usage: bash scripts/armnosem_chain.sh [node ...]   (default: all nodes)
set -uo pipefail
...
ST=artifacts/runs/ladder_armnosem_state; mkdir -p $ST
C=configs/ladder/armnosem
RUNS=artifacts/runs
...
REP0=$RUNS/ladder_latent_nosem_b1fix_anchor/representation.pt
F0=$RUNS/ladder_flow_nsjf/snap_final_s20000.pt
FFT=$RUNS/ladder_flow_nsjf_ft/policy.pt
FG1=$RUNS/ladder_flow_nsjf_gdag1/policy.pt
FG2H=$RUNS/ladder_flow_nsjf_gdag2h/policy.pt
rz() { echo $RUNS/ladder_rz_nsjf_$1/representation.pt; }
...
collect() {  # name rep seed [flow] [genctx]: 4 parallel one-shot jobs, then verify all 13 buffers
  local name=$1 rep=$2 seed=$3 flow=${4:-} gc=${5:-} out=$RUNS/ladder_dagger_nsjf_$1 pids=() i=0 ok=0
  for g in "$G1" "$G2" "$G3" "$G4"; do
    i=$((i+1))
    ops --cpu 3 --mem 8G --label ans_col_${name}_$i --max-seconds 10800 -- env EXPERT=bc ${flow:+FLOW=$flow} ${gc:+GENCTX=1} SEED=$seed \
...
r2eval() {  # tag flow rep seedstart robots...: R2 evaluations (CPU)
  ...
  local rs="$*"
  ops --cpu 3 --mem 8G --label ans_r2_${tag}_$s --max-seconds 10800 -- bash -c "export CUDA_VISIBLE_DEVICES=; set -e; sha256sum $flow $rep; for r in $rs; do $PY scripts/ladder.py --route generated --flow $flow --rep $rep --prev-action zero --robot \$r --n 30 --seed-start $s --tag zero_${tag}_s$s --out artifacts/runs/ladder_v1/\$r; done"
}
orcbc() {  # rep tag: stateless R1 oracle diagnostic
  ops --cpu 3 --mem 10G --label ans_orcbc_$2 --max-seconds 10800 -- bash -c "set -e; bash scripts/ladder_eval_orcbc.sh $1 $2; for r in panda_pg2 parm6_tf3; do test -f artifacts/runs/ladder_v1/\$r/oracle_zero_$2_orcbc.summary.json; done"
}
semedit() {  # robot seedstart episodes shardname: task-context semantic-edit suite (D-074/075/077 command)
  ops --cpu 1 --mem 2G --label ans_sem_${4//\//_} --max-seconds 14400 -- env OMP_NUM_THREADS=1 $PY -m rrp.cli latent semantic-edits --route generated \
    ...
    --out artifacts/runs/acceptance_armnosem_gen_$4
}
semedits_all() {
  local pids=() ok=0
...
final_evals() {
  local pids=() ok=0 T=flownsgdag2h_rznsgendag3_noqd
...
prog_evals() {  # progression rows on the dev seeds (same pairs as the sem progression table)
  local pids=() ok=0
  r2eval flowns20k_rznsgendag1_noqd $F0 $(rz gendag1_noqd) 3000000 parm6_tf3 panda_pg2 & pids+=($!)
  r2eval flownsgdag1_rznsgendag3_noqd $FG1 $(rz gendag3_noqd) 3000000 parm6_tf3 panda_pg2 & pids+=($!)
  orcbc $(rz gendag3_noqd) nsjfgendag3noqd & pids+=($!)
  for p in "${pids[@]}"; do wait $p || ok=1; done
  return $ok
}

flow0() {  # as the sem watcher did: snap_final_s20000.pt = copy of the final policy.pt (warm-start path in the ft/gdag1 configs)
  train_gpu ans_flow0 $C/flow_nsjf.json flow 16G 14400 && cp $RUNS/ladder_flow_nsjf/policy.pt $F0
}

# ---- DAG ----
all_nodes() {
  node stageA - train_gpu ans_stageA $C/rep-latent_nosem_b1fix_anchor.json rep 16G 14400 &
  node F0 stageA flow0 &
  node Fft F0 train_gpu ans_flowft $C/flow_nsjf_ft.json flow 16G 10800 &
  node bc1 stageA collect bc1 $REP0 3200000 &
  node rzbcdag1 bc1 train_gpu ans_rz_bcdag1 $C/rz_nsjf_bcdag1.json rz 8G 7200 &
  node rzbcdag1long bc1 train_gpu ans_rz_bcdag1long $C/rz_nsjf_bcdag1_long.json rz 8G 10800 &
  ...
  node rzbcdag2 rzbcdag1long,bc2 train_gpu ans_rz_bcdag2 $C/rz_nsjf_bcdag2.json rz 8G 10800 &
  node rzgendag1 rzbcdag2,gen1,bc3 train_gpu ans_rz_gendag1 $C/rz_nsjf_gendag1_noqd.json rz 8G 7200 &
  node semedits rzgendag1,F0 semedits_all &
  node gen2 rzgendag1,F0 collect gen2 $(rz gendag1_noqd) 3700000 $F0 &
  node rzgendag2 gen2 train_gpu ans_rz_gendag2 $C/rz_nsjf_gendag2_noqd.json rz 8G 7200 &
  ...
  node rzgendag3 gen3 train_gpu ans_rz_gendag3 $C/rz_nsjf_gendag3_noqd.json rz 8G 7200 &
  node Fgdag1 gdag1,F0 train_gpu ans_flowgdag1 $C/flow_nsjf_gdag1.json flow 16G 7200 &
  node gdag2 rzgendag3,Fgdag1 collect gdag2 $(rz gendag3_noqd) 4000000 $FG1 1 &
  node Fgdag2h gdag2,Fgdag1 train_gpu ans_flowgdag2h $C/flow_nsjf_gdag2h.json flow 16G 7200 &
