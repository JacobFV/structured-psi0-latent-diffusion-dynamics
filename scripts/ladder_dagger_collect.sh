#!/usr/bin/env bash
# System-0 DAgger collection: R1 (re-anchored oracle) rollouts of the CURRENT realizer on source-train bodies,
...
for r in "$@"; do
  ...
  $PY scripts/ladder.py --route oracle --reanchor --prev-action zero --robot $r --n $N --seed-start 3200000 \
