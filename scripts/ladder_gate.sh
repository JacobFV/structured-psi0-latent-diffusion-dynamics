...
# 30 matched dev seeds. Gate metric: sys0 arm error / hold-still arm error (target <= 0.20). Usage: ladder_gate.sh <rep.pt> <tag>
set -uo pipefail
REP=$1; TAG=$2
...
for r in panda_pg2 parm6_tf3; do
  $PY scripts/ladder_localize.py --policy artifacts/runs/baselines_bc_ckpts/direct1701_u12000.pt --policy-label direct1701_u12000 \
    --rep $REP --robot $r --n 30 --out artifacts/runs/ladder_localize/$r/bc_direct1701_u12000__$TAG.json
done
