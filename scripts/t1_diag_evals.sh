...
PAR=${PAR:-5}
case $1 in
  refit) for v in ${2:-sem nosem}; do for k in ${KS:-genz ctl}; do
  ...
  lv4) bash scripts/legged_ladder.sh t1 t1diag_sem_lv4 $PAR r2 - - - artifacts/runs/t1diag_flow_sem_lv4/policy.pt &
