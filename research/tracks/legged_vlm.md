...
| flow go2 (host GPU lease 1790437089_a86b45, `BODIES=go2 bash scripts/legged_fixsem_train.sh`, rc=0) | completed | `artifacts/runs/legged_fixsem_flow_sem_go2_lv4/result.json`: held-out realization with oracle / generated packets 0.0145 / 0.0166 (orig sem 0.0192 / 0.0209; nosem 0.0129 / 0.0162) |
| hexapod6 Stage A (peer GPU) | completed | `artifacts/runs/legged_fixsem_rep_sem_hexapod6_lv4/result.json`: realization MSE 9.7e-5 (orig sem 1.8e-4, nosem 6.4e-5; zero-action 0.086); probes goal 0.0136, disp xy 0.039 / yaw 0.038, subtask 1.0, contact 0.67 / swing 0.36 (orig 0.55 / 0.36) |
...
| go2 flows moved to the host (11:45) | running | With 7 processes the peer GPU ran at ~0.5 s/step. Once the 3 peer go2 chains had finished Stage A and the nosem probe, `scripts/legged_fixrep_move_go2flows.sh` stopped peer lease 1790440106_9ac1a9 (`ops stop --owned-only --lease`), discarded the flows that had just started there, and retrained those 3 flows FROM SCRATCH on host GPU lease 1790447816_10e23c. Stage A is unchanged (trained on the peer). Eval auto-launch: `legged_fixrep_orchestrate.sh go2hostflows`. |
| nosem go2 s1 | completed | host lease 1790440021_55a138 (rc=1 overall, but this chain finished); eval peer lease 1790442956_641734 rc=0 |
...
| evals | planned (auto-launched by `scripts/legged_fixrep_orchestrate.sh {hex,go2peer,go2host}` when each training lease ends with its flows present) | peer CPU |
**Incident (09:40).** I ran `scripts/peer_sync.sh push` once without `RRP_PEER_REPO`. It rsynced this worktree, which was current origin/main, into the SHARED peer code dir `/dev/shm/rrp-brandonin/repo` with `--delete`. The sync excludes artifacts, ops state, .git and resources.local.json. Any uncommitted files that existed only in that code dir were deleted; I cannot tell what, if anything, was lost. The only process running from that dir was the peer watchdog, whose modules were already loaded. Reported to the lead.
